...
- **Part 1**: A Jupyter Notebook (`part1.ipynb`) that enhances a Connecticut real estate sales dataset (2001–2020) by filtering for Mondays in April 2019, geocoding addresses using Nominatim, and fetching weather data using Open Meteo’s Archive API.
...
- **Dataset**: `Real_Estate_Sales_2001-2020.csv` (committed to the repository).
- **Tasks**:
  - Filtered real estate data for Mondays in April 2019 using Pandas.
  - Geocoded addresses missing latitude/longitude with Nominatim’s public API, caching results to respect rate limits (`time.sleep(1)`).
...
- **Output**: `part1.ipynb`, `enhanced_real_estate_april_2019.csv` (not committed, generated by notebook).
